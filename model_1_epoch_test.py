...
def main():
    epoch_values = [5, 10, 15, 25, 50]          #list of epochs to test
    ...
    for num_epochs in epoch_values:             #loop over all epoch options from list
        ...
        scheduler = optim.lr_scheduler.StepLR(optimizer, step_size = 10, gamma = 0.5)               #reudce learning rate by half ever 10 epochs

        ############################################################################
        # Training Loop
        ############################################################################
        for epoch in range(CONFIG["epochs"]):                                                           #loop through epochs in epoch list
            train_loss, train_acc = train(epoch, model, trainloader, optimizer, criterion, CONFIG)      #train model for singular epoch
            val_loss, val_acc = validate(model, valloader, criterion, CONFIG["device"])                 #validate model on validation dataset
            scheduler.step()                                                                            #step the scheduler

            # Save the best model (based on validation accuracy)
            if val_acc > best_val_acc:                                  #if current model set up has higher validation accuracy, save it
                best_val_acc = val_acc                                  #update best validation value
                best_config = {"epochs": num_epochs}                    #save best performing epoch value
                torch.save(model.state_dict(), "best_model.pth")        #save model to file
        
        print(f"Training Completed with epochs {num_epochs} and val accuracy = {val_acc:.2f}%")
        results.append({"epochs": num_epochs, "val_acc": val_acc})

    print("\nBest Model Config:")
    print(best_config)
    print(f"Best Validation Accuracy: {best_val_acc:.2f}%")
    # === Final Evaluation
    final_model = SimpleCNN().to(CONFIG["device"])
    ...
    all_predictions = eval_ood.evaluate_ood_test(final_model, CONFIG)
